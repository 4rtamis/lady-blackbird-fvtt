...
Hooks.once('init', function () {
  ...
  Actors.registerSheet('lady-blackbird', LadyBlackbirdActorSheet, {
    makeDefault: true,
    label: 'LADY_BLACKBIRD.SheetLabels.Actor',
  });
  Items.unregisterSheet('core', ItemSheet);
  Items.registerSheet('lady-blackbird', LadyBlackbirdItemSheet, {
    makeDefault: true,
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.ladyblackbird.rollItemMacro("${data.uuid}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { 'lady-blackbird.itemMacro': true },
    });
  }
